...
## 版面架構（1920×1080 固定設計尺寸）

JS 在啟動時對 `#tbl` 套用 `transform: scale()` 讓它適應任意視窗。

```
┌──────────────────────────────────────────────────────┐  y=0
│  #pa2-area  (top, 173px)  — 對家 p2（西家）         │
│  #pa3-area  (left, 300px wide)  — 左家 p3（北家）   │  y=173
│                                                      │
│        #pool  (1320×645, left:300 top:173)           │
│          ┌──────────────────────────────┐            │
│          │  #dc2  (p2 棄牌，top zone)   │            │
│          │  .cp  (center circle)         │            │
│          │  #dc3  #dc1  (left/right)     │            │
...
│          └──────────────────────────────┘            │
│  #pa1-area  (right, 300px wide)  — 右家 p1（南家）  │  y=802
│  #p0-zone  (bottom, 345px)  — 自己 p0（東家）       │
└──────────────────────────────────────────────────────┘  y=1080
```
